...
class NeuralNetwork(object):
    ...
    def train(self, features, targets):
        ...
        for X, y in zip(features, targets):
            ...
            hidden_error_term = np.dot(self.weights_hidden_to_output , output_error_term) *             self.activation_function(hidden_inputs) * (1-self.activation_function(hidden_inputs))
...
# #### 请将你的答案填写在下方
# 
# 模型不够复杂吧，增加节点数不如增加层数有用，此外没有能够利用上时间序列信息的结构，如果变成RNN可能会好一点？
# 另外就是数据本身也不是百分百能被一个模型预测的，数据本身就有不确定性，输入和输出之间不具有完全性的决定性。极端一点来说，比方说你永远无法用一个模型去拟合一组随机生成的数据并且准确预测。
# 
# 其实loss还在降，尽管有点振动（需要调低学习率了，但是一开始调低就收敛太慢，我又不能手写一个动量机制……)，但是我笔记本没电了，懒得继续训练了。
