...
def text_to_ids(source_text, target_text, source_vocab_to_int, target_vocab_to_int):
    ...
    for line in source_text.split('\n'):
        int_line = [source_vocab_to_int.get(word, source_vocab_to_int['<UNK>']) for word in line.split()]
        source_id_text.append(int_line) 
...
def model_inputs():
    ...
    return inputs_, target, lr, keep_prob
...
def decoding_layer_train(encoder_state, dec_cell, dec_embed_input, sequence_length, decoding_scope,
    ...
    train_pred, _, __ = tf.contrib.seq2seq.dynamic_rnn_decoder(dec_cell,                                                               train_dec_fn,dec_embed_input,sequence_length,scope=decoding_scope) 
...
def decoding_layer_infer(encoder_state, dec_cell, dec_embeddings, start_of_sequence_id, end_of_sequence_id,
    ...
    logits, _, __ = tf.contrib.seq2seq.dynamic_rnn_decoder(dec_cell,infer_decoder_fn,scope=decoding_scope)
    return logits
...
#     print(vocab_size)
    ...
    # DropOut Wrapper 在train和infer两个函数里面加
    # 不明白为什么infer的时候要dropout呢？
    with tf.variable_scope('decoding') as decoding_scope:
    ...
    with tf.variable_scope('decoding',reuse=True) as decoding_scope:
        ...
        infer_logits = decoding_layer_infer(encoder_state,cell,dec_embeddings,target_vocab_to_int['<GO>'],                                           target_vocab_to_int['<EOS>'],sequence_length,vocab_size,decoding_scope,output_fn,keep_prob)
...
def seq2seq_model(input_data, target_data, keep_prob, batch_size, sequence_length, source_vocab_size, target_vocab_size,
    ...
    #实在不知道什么情况下要用scope，头晕。
#     with tf.VariableScope('decoding') as decoding_scope:
#         decoder_embedding_weight = tf.get_variable('embedding_weight',[target_vocab_size,dec_embedding_size],
#                                                    dtype=tf.float32,initializer=tf.random_uniform_initializer())
    decoder_embedding_weight = tf.Variable(tf.random_uniform([target_vocab_size,dec_embedding_size]),dtype=tf.float32)

    decoder_embed_input = tf.nn.embedding_lookup(decoder_embedding_weight,decoder_input)
    
    
    
    
    train_logits,infer_logits = decoding_layer(decoder_embed_input,decoder_embedding_weight,encoder_state,
